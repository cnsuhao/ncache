...
int ngx_http_hash_check_count(int hash_index);
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
	...
	else if(hash_stat == 1)
	{
		...
		int index = atoi(&nhucht[hash_index].cache.index);
...
static ngx_int_t ngx_http_shm_hash_init()
{
	...
	if(hash_fd < 0)
	{
		...
		lseek(hash_fd, sizeof(ngx_http_upstream_cache_hash_t)*ALL_CACHE_NUM-1, SEEK_SET);	
		write(hash_fd ,"", 1);
	...
	if(count_fd < 0)
	{
		...
		lseek(count_fd, sizeof(ngx_http_upstream_cache_hash_count_t), SEEK_SET);	
		write(count_fd ,"", 1);
	...
	if(nhuchct == MAP_FAILED)
	{
		printf("ngx_http_shm_hash_init count mmap error\n");
		return NGX_ERROR;
	}
	...
	if(nhucht == MAP_FAILED)
	{
		printf("ngx_http_shm_hash_init mmap error\n");
		return NGX_ERROR;
	}
	...
	if(!file_exist)
	{
		...
		msync(nhucht, sizeof(ngx_http_upstream_cache_hash_t)*ALL_CACHE_NUM, MS_ASYNC);
	}
	
	if(!count_exist)
	{
		memset(nhuchct, 0, sizeof(ngx_http_upstream_cache_hash_count_t));
		msync(nhuchct, sizeof(ngx_http_upstream_cache_hash_count_t), MS_ASYNC);
...
ngx_int_t ngx_http_shm_is_timeout(ngx_file_info_t *fi, int hash_index)
{
	...
	if(time(NULL) - ngx_file_mtime(fi) > nhucht[hash_index].cache.time)
	{
		writew_lock(hash_fd, hash_index * cache_size, SEEK_SET, cache_size);
		nhucht[hash_index].cache.timeout |= 0x01;
		un_lock(hash_fd, hash_index * cache_size, SEEK_SET, cache_size); 
...
void ngx_http_shm_hash_update(int hash_index, u_char *key, time_t time, u_char type, int zip, u_char charset)
{
...
ngx_int_t ngx_http_shm_hash_add(int hash_index, u_char *key, int dir_index)
{
	sprintf(&nhucht[hash_index].cache.index, "%d", dir_index);
...
ngx_http_upstream_purge_handler(ngx_http_request_t *r)
{
	...
	//ngx_str_t key = ngx_string("purge_uri");
	//ngx_uint_t h_key = ngx_hash_key(key.data, key.len);
	//ngx_http_variable_value_t *nhvvt = ngx_http_get_variable(r, &key, h_key, 1);
	...
	if(hash_stat == 1)
	{
		...
		int index = atoi(&nhucht[new_index].cache.index); 
	...
	sprintf(content, "<html><body>%s</body></html>", request_path);

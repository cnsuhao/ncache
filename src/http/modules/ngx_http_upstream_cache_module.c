...
extern void ngx_http_shm_hash_update(int new_index, u_char *key, time_t time, u_char type, int zip, u_char charset);
...
void ngx_http_upstream_cache_set_content_type(ngx_http_request_t *r, VILA *v)
{
	if(r->headers_out.content_type.data)
	{
		...
		{
			if(!strcmp(r->headers_out.charset.data, "utf-8"))
			{
				v->charset = '1';
			}
		}
...
ngx_http_upstream_cache_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
 	...
 	if(in->buf->last_buf) 
	{
		...
		int index = atoi(&nhucht[r->cache_hash_index].cache.index);
		...
		{
			if(!strncmp(ccp[j]->value.data, "max-age=", 8))
			{
...
cachenext:
		{
			...
			while(tmp && tmp->buf && tmp->buf->pos)
			{
				...
				memcpy(str+allen, tmp->buf->pos, len);
				allen += len;

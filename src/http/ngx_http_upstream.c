...
static char *ngx_http_cache_dir(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
static ngx_command_t  ngx_http_upstream_commands[] = {
	...
	//add a cachedir command in nginx config file
	{ ngx_string("cachedir"),
      NGX_HTTP_UPS_CONF|NGX_CONF_TAKE3,
      ngx_http_cache_dir,
      NGX_HTTP_SRV_CONF_OFFSET,
      0,
      NULL },
...
//ncache set the cache dir info, must same as your mkdir_ngx_cache.sh
static char *
ngx_http_cache_dir(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{


	ngx_http_upstream_srv_conf_t  *uscf = conf;

    //greate a ngx_http_upstream_cache_conf_t array to save every cache dir info
	if (uscf->caches == NULL) {
        uscf->caches = ngx_array_create(cf->pool, 4,
                                         sizeof(ngx_http_upstream_cache_conf_t));
        if (uscf->caches == NULL) {
            return NGX_CONF_ERROR;
        }
    }

	ngx_http_upstream_cache_conf_t *uc = ngx_array_push(uscf->caches);

	if (uc == NULL) {
        return NGX_CONF_ERROR;
    }

    ngx_memzero(uc, sizeof(ngx_http_upstream_cache_conf_t));
    ...
    //read the cache dir info form config file and save it
	for (i = 1; i < cf->args->nelts; i++)
	{
		...
		if(i == 1)
		{
			uc->path.data = ngx_palloc(cf->pool, value[i].len + 1);
			uc->path.data[value[i].len] = 0;
			uc->path.len = value[i].len;
			ngx_cpymem(uc->path.data, value[i].data, value[i].len);
		}
		if(i == 2)
		{
			uc->size1 = ngx_atoi(value[i].data, value[i].len);
		}
		if(i == 3)
		{
			uc->size2 = ngx_atoi(value[i].data, value[i].len);
		}
